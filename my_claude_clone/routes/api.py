...
def post_message(cid):
    ...
    if web_search_enabled:
        ...
        if search_results:
            system_parts.append(
                "Ниже результаты поиска Google по запросу пользователя. "
                "Используй эту актуальную информацию в ответе, если это уместно, "
                "и ссылайся на источники:\n\n" + search_results
            )
        else:
            system_parts.append(
                "Пользователь включил Google Search, но поиск не вернул результатов "
                "(проверь GOOGLE_API_KEY и GOOGLE_CSE_ID в .env). Ответь на основе своих знаний."
            )
